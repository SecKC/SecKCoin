...
uint256 hashGenesisBlock("0xb9ee3576bf4ebaf0878d1f5bb02905d2d38ee9b90e08134719f002328cce07d7");
...
const string strMessageMagic = "SecKCoin Signed Message:\n";
...
{
    // SecKCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 133700000;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // SecKCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 302400; // SecKCoin: 3.5 days
static const int64 nTargetSpacing = 86; // SecKCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SecKCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SecKCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // SecKCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xee4297b75c537bb2498df010cace9e9b01eeaaa95c76ec878b43dcbb8f1da38a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SecKC DOPE COIN FOR DOPE SHIT";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 133700000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("7336f569c57a813cbc1df0a8eaac1719307fcf81787df62b4d2cd660bc8fea0217f6db6e8b5a154da0f1910fabd380524e2fd25cf58a12fd8c38fc56e1bb655a4b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1464452501;
        block.nBits    = 504365644;
        block.nNonce   = 2311811609;

        if (fTestNet)
        {
            block.nTime    = 1464452501;
            block.nNonce   = 2048763176;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xaf20ca52ab01f38ea2784c88bcb344e5ecf08c67f6405cabd142a598c1523009"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // SecKCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SecKCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SecKCoinMiner : ProcessBlock, block not accepted");
    }

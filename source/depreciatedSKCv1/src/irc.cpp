...
// Copyright (c) 2011-2012 SecKCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SecKCoinTEST0\r");
            Send(hSocket, "WHO #SecKCoinTEST0\r");
        } else {
            // randomly join #SecKCoin00-#SecKCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #SecKCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SecKCoin%02d\r", channel_number).c_str());
        }
